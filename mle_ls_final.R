...
### SH details 
sh_master = gcs_get_object("ipc/Nav/IPC_Hyperlocal/DS_Network_Master.csv", bucket = gcs_bucket2)
setDT(sh_master)
sh_master <- sh_master[, .(DS, SH = fifelse(!is.na(SH_P1), SH_P1, SH_P0))]
setnames(sh_master, old =c('SH'),new=c('sh_code'))


##Adding SH codes to selection master(mle+Ls)
selection_master_ds = merge(selection_master_ds,sh_master, by.x = c('business_zone'),by.y = c('DS'), all.x = TRUE)
...
stn_creation_sh=merge(stn_creation_sh,sh_master, by.x = c('business_zone'),by.y = c('DS'),all.x=T)
