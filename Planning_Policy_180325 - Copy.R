...
####reading top 2500
top_2500<-gcs_get_object("ipc/Nav/IPC_Hyperlocal/top_2500.csv", bucket = gcs_bucket2)
#top_2500<-read.csv("temp_file.csv")
setDT(top_2500)
top_2500 = top_2500[,.(fsn)]
top_2500 = unique(top_2500)
...
## reading stressed DS
ds_choked<-gcs_get_object("ipc/Nav/IPC_Hyperlocal/ds_choked_input.csv", bucket = gcs_bucket2)
setDT(ds_choked)
ds_choked = ds_master[ds_choked, on=c('DS')]
ds_choked= ds_choked[,.(Cluster)]
ds_choked= unique(ds_choked)
...
ds_master_mle = ds_master[,`:=`(use_case_e = ifelse(Cron_Timing %in% c("ME","E"),"nl_ds2",""))]
...
setDT(Rep_Master)
Rep_Master = Rep_Master[,`:=`(top_2500 = ifelse(fsn %in% unique(top_2500$fsn),1,0))]
Rep_Master = Rep_Master[,`:=`(choked_store = ifelse(cluster %in% unique(ds_choked$Cluster),1,0))]
Rep_Master = Rep_Master[,`:=`(inventory_target = ifelse(choked_store ==1 & top_2500 == 0,0,inventory_target))]
Rep_Master = Rep_Master[,`:=`(min_inventory_target = ifelse(choked_store ==1 & top_2500 == 0,0,min_inventory_target))]
Rep_Master = Rep_Master[,`:=`(top_2500 = NULL,choked_store = NULL)]
#####Trop Master
...
setDT(IWIT_Master)
iwit_3 = IWIT_Master[use_case %in% c('nl_ds','nl_ds2')]
iwit_12 =IWIT_Master[!use_case %in% c('nl_ds','nl_ds2')]
...
####Binola & DIC destination patch end
...
temp[,`:=`(check = ifelse(fsn %in% unique(binola_fsn$fsn) & cluster == 'CLUS_SFX_bin_sh',1,0))]
temp = temp[check == 0]
temp[,`:=`(check = NULL)]
...
##removing min-max for Casepack fsns###########################
