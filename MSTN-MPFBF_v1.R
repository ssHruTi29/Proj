...
####### Creating the base key from ATP
...
colnames(P_seller_list)[1]<-"Seller_id"
...
### final unique_LIDs from Sale and ATP
...
####mapping BU,cluster and zone to the key file
...
###Change date as of the day of run(it is used to filter the scheduling data for forward schedules)
...
for (i in all_weeks)
{
  ...
  ####Change this basis the largest size in files dataframe
...
Total_key_summary_national<-merge(Total_key_summary_national,P_seller_list,by.x=c("Seller_id"),by.y=c("Seller_id"),all.x=T)
Total_key_summary_national$P_seller[is.na(Total_key_summary_national$P_seller)]<-0
...
write.csv(Total_key_summary_national,"C:\\Users\\shruti.shahi\\Desktop\\BGM\\Analytics\\Output\\MP-MSTN_15Jul_national_v1.csv",row.names = FALSE)


##Zonal#####################################################################################

#############instock for speed calculation at fsn level instock
###############################################################################################3
...
Total_key_summary_zone_fsn<-merge(Total_key_summary_zone_fsn,P_seller_list,by.x=c("Seller_id"),by.y=c("Seller_id"),all.x=T)
Total_key_summary_zone_fsn$P_seller[is.na(Total_key_summary_zone_fsn$P_seller)]<-0
...
Total_key_summary_zone<-merge(Total_key_summary_zone,P_seller_list,by.x=c("Seller_id"),by.y=c("Seller_id"),all.x=T)
Total_key_summary_zone$P_seller[is.na(Total_key_summary_zone$P_seller)]<-0
...
write.csv(Total_key_summary_zone,"C:\\Users\\shruti.shahi\\Desktop\\BGM\\Analytics\\Output\\MP-MSTN_15Jul_zonal_v1.csv",row.names = FALSE)
