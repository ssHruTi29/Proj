...
atp = gcs_get_object("ipc/Nav/Hyperlocal_IPC/Inventory_Snapshot.csv", bucket = gcs_bucket,saveToDisk = "temp_file.csv",overwrite = TRUE)
atp<-read.csv("temp_file.csv")
setDT(atp)
atp = atp[fc_area == 'store']
Inv = atp[,.SD]

ds_master = gcs_get_object("ipc/Nav/IPC_Hyperlocal/DS_Master.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
facility_mapper = gcs_get_object("ipc/Nav/IPC_Hyperlocal/Facility_Mapper_Dump.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
###Reading instock DRR
instock_corrected_drr = gcs_get_object("Avinash_HL_Data/Instock_CorrectedDRR_14Days.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
selection_master_ds = gcs_get_object("ipc/Nav/IPC_Hyperlocal/mle_master.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
selection_master_ds<-read.csv("temp_file.csv")
setDT(selection_master_ds)

lifestyle_master_ds = gcs_get_object("ipc/Nav/IPC_Hyperlocal/lifestyle_master.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
lifestyle_master_ds = lifestyle_master_ds[,`:=`(date ='',SAMs_Club ='No')]
lifestyle_master_ds = lifestyle_master_ds[,.(business_zone,FSN,date,City,BU,SC,Analytical_Vertical,brand,Title,NOOS,SAMs_Club,Buy_POC,flag,cu)]
...
### add NL details
###NL - set 1(First FCs)
nl_master = gcs_get_object("ipc/Nav/IPC_Hyperlocal/nl_hub.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
####NL - set 2 (Second FCs)
nl_master2 = gcs_get_object("ipc/Nav/IPC_Hyperlocal/nl_hub2.csv", bucket = gcs_bucket2,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
####Mapping SH to DS
ds_it = gcs_get_object("ipc/Nav/Hyperlocal_IPC/DS_IT.csv", bucket = gcs_bucket,saveToDisk = "temp_file.csv",overwrite = TRUE)
...
# ds_iwit_n_1 = ds_it[Creation_Date != Sys.Date()]
# 
# ds_iwit_pending_dispatch = ds_iwit_n_1[Dispatch_Status == 'RESERVED']
# ds_iwit_pending_dispatch = ds_iwit_pending_dispatch[,.(pending_disptach = sum(IWIT_Intransit)), by =.(fsn,Dest_FC)]
# 
# ds_iwit_dispatch = ds_iwit_n_1[Dispatch_Status == 'DISPATCHED']
# ds_iwit_dispatch = ds_iwit_dispatch[,.(dispatch = sum(IWIT_Intransit)), by =.(fsn,Dest_FC)]
# 
# ds_iwit_n_1 = ds_iwit_n_1[,.(iwit_n_1 = sum(IWIT_Intransit)), by =.(fsn,Dest_FC)]

setnames(ds_iwit, old =c('fsn','Dest_FC','Src_FC'),new=c('FSN','business_zone','sh_code'))
# setnames(ds_iwit_n_1, old =c('fsn','Dest_FC'),new=c('FSN','business_zone'))
# setnames(ds_iwit_pending_dispatch, old =c('fsn','Dest_FC'),new=c('FSN','business_zone'))
# setnames(ds_iwit_dispatch, old =c('fsn','Dest_FC'),new=c('FSN','business_zone'))
...
####Adding DS Norms 
ds_norms = gcs_get_object("ipc/Nav/Hyperlocal_IPC/DS_Target_Master.csv", bucket = gcs_bucket,saveToDisk = "temp_file.csv",overwrite = TRUE)
